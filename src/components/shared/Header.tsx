...
export default function Header({
	...
	const navAnchorIds = useMemo(
		() =>
			NAV_ITEMS.map((item) => getAnchorId(item)).filter((value): value is string => Boolean(value)),
		[],
	);
	...
	const navLinks = useMemo<HeaderNavLink[]>(
		() =>
			NAV_ITEMS.map((item) => {
				...
				const routerHref =
					"href" in item && typeof item.href === "string" ? item.href : (anchorHref ?? "#");
	...
	return (
		<header
			ref={headerRef}
			className={[
				"sticky top-0 z-50 border-t-2.5 border-b-2 border-b-scroll-100 transition-colors duration-500 motion-reduce:transition-none",
				theme.background,
				theme.borderAccent,
				theme.text,
			].join(" ")}
...
function HeaderMobile({ theme, isMenuOpen, panelId, onToggleMenu, triggerRef }: HeaderMobileProps) {
	return (
...
function Logo({ variant, logoPath }: LogoProps) {
	const baseClasses = "w-auto transition-all duration-300 motion-reduce:transition-none";

	const sizeClasses = variant === "tall" ? "h-[150px]" : variant === "thin" ? "h-12" : "h-10";
...
const MenuButton = forwardRef<HTMLButtonElement, MenuButtonProps>(function MenuButtonComponent(
	...
	return (
		<button
			...
			onClick={onToggle}
			className={[
				"inline-flex items-center justify-center rounded-md px-3 py-2 transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 motion-reduce:transition-none",
				theme.navHover,
			].join(" ")}
		>
...
function MobileMenuPanel({
	...
	return (
		...
		>
			<div
				className={[
					"type-display-md w-full max-w-[16rem] origin-top overflow-hidden rounded-b-md px-4 py-6 transition-all duration-300 motion-reduce:transition-none",
					theme.menuSurface,
					isOpen ? "mt-2 scale-y-100 opacity-100" : "pointer-events-none mt-0 scale-y-95 opacity-0",
				].join(" ")}
			>
				<NavLinksComponent
					theme={theme}
...
function getAnchorId(item: NavItem): string | null {
	if ("anchor" in item && typeof item.anchor === "string") {
		const normalized = item.anchor.startsWith("#") ? item.anchor.slice(1) : item.anchor;
...
function useIsDesktop(): boolean {
	...
	useEffect(() => {
		...
		const listener = (event: MediaQueryListEvent) => applyMatchState(event.matches);
...
function useMobileMenu({ panelRef, triggerRef }: UseMobileMenuOptions): MobileMenuState {
