...
>(({ className, ...props }, forwardedRef) => {
	...
	return (
		<div
			ref={forwardedRef}
			className={cn("type-body-md py-6 text-center", className)}
			cmdk-empty=""
			role="presentation"
			{...props}
		/>
		...
		return (
			...
			>
				<div
					...
					className={cn(
						"type-body-md flex items-start justify-between rounded-md border border-moloch-800 p-0.5 ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2",
						{
				...
				>
					<div className="relative flex w-full flex-col gap-0.5">
						{selected.map((option) => {
							return (
								<Badge
									key={option.value}
									className={cn(
										"type-body-md w-full justify-between rounded-md px-2 py-1 text-scroll-100",
						...
						{Object.keys(selectables).some((key) => selectables[key].length > 0) && (
							<CommandPrimitive.Input
								...
								className={cn(
									"type-body-md flex-1 self-baseline bg-transparent px-2 py-1 font-body outline-none placeholder:text-moloch-800/60",
									{
				...
				<div className="relative">
					{open && Object.keys(selectables).some((key) => selectables[key].length > 0) && (
						<CommandList
