		...
		<Card className={cn("", className)}>
			...
			<CardContent>
				<ResponsiveContainer width="100%" height={height}>
					<LineChart data={data}>
						<CartesianGrid strokeDasharray="3 3" stroke={chartColors.neutral} opacity={0.3} />
						<XAxis dataKey={xKey} stroke={chartColors.neutralLight} fontSize={12} />
...
export function ChartGrid({ charts, className }: ChartGridProps) {
	...
	return (
		<div className={cn("grid grid-cols-1 gap-6 md:grid-cols-2", className)}>
			{charts.map((chart) => (
				<div key={chart.config.title}>{renderChart(chart.type, chart.config)}</div>
			))}
		</div>
	);
}
